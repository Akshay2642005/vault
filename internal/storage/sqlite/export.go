...
func (b *Backend) exportVault(ctx context.Context) ([]byte, error) {
	...
	secrets, vers, err := b.exportSecrets(ctx)
	if err != nil {
		return nil, err
	}
...
func (b *Backend) exportSecrets(ctx context.Context) (map[string][]*storage.SecretDump, map[string][]*storage.VersionDump, error) {
	rows, err := b.db.QueryContext(ctx, `
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &projectID, &s.Environment, &s.Key, &value,
			...
			rows.Close()
			return nil, nil, fmt.Errorf("failed to scan secret: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		rows.Close()
		return nil, nil, fmt.Errorf("failed to iterate secrets: %w", err)
	}
	rows.Close()

	// Versions
	vv, err := b.exportVersions(ctx)
	if err != nil {
		return nil, nil, err
	}

	return secrets, vv, nil
}
...
func (b *Backend) importVault(ctx context.Context, data []byte) error {
	...
	// Wipe existing data (cascading FKs remove children; FTS triggers stay in sync)
	...
	for _, p := range dump.Projects {
		...
		for _, s := range p.Secrets {
			...
			`, s.ID, p.ID, s.Environment, s.Key, s.Value, s.Type, s.Tags, s.Metadata,
		...
		for _, s := range p.Secrets {
			for _, v := range s.Versions {
				if _, err := tx.ExecContext(ctx, `
					...
					VALUES (?, ?, ?, ?, ?, ?, ?)
				`, v.ID, s.ID, v.Value, v.Version, v.CreatedAt, v.CreatedBy, v.Checksum); err != nil {
	...
	for _, p := range dump.Projects {
		for _, s := range p.Secrets {
			...
			if _, err := tx.ExecContext(ctx, `UPDATE secrets SET previous_id = ? WHERE id = ?`,
